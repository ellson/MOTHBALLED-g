...
static elem_t *
new_elem_sub (elemtype_t type)
{
  elem_t *elem, *next;
  int i;

  if (!free_elem_list)
    {				// if no elems in free_elem_list
...
elem_t *
new_frag (char state, int len, unsigned char *frag, inbuf_t * inbuf)
{
  elem_t *elem;
...
static elem_t *
clone_list (char state, elem_t * list)
{
  elem_t *elem;
...
elem_t *
move_list (char state, elem_t * list)
{
  elem_t *elem;
...
elem_t *
ref_list (char state, elem_t * list)
{
  elem_t *elem;
...
void
push_list (elem_t * list, elem_t * elem)
{
  assert (list->type == (char) LISTELEM);
...
void
print_frag (FILE * chan, unsigned char len, unsigned char *frag)
{
  while (len--)
    putc (*frag++, chan);
}
...
int
print_len_frag (FILE * chan, unsigned char *len_frag)
{
  unsigned char len;
...
void
print_list (FILE * chan, elem_t * list, int indent, char sep)
{
  elem_t *elem;
  elemtype_t type;
  unsigned char *cp;
