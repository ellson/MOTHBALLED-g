...
<ACT VERB>[OPT]<= SUBJECT><= ATTRIBUTES>[OPT]<= CONTAINER>[OPT]<= TERM>[OPT]
<VERB TLD>[ALT]<= QRY>
...
<SIBLING NODEREF><= CHILD>[OPT REP]<= PORT>[OPT]<= ATTRIBUTES>[OPT]
<COUSIN PARENT>[REP]<= CHILD>[REP]<= PORT>[OPT]<= ATTRIBUTES>[OPT]
<PARENT HAT><= FSL>
<CHILD FSL><= NODEREF>
