...
more_in (context_t * C)
{
  ...
  if (C->inbuf)
    {				// if there is an existing active-inbuf
      if (C->in == &(C->inbuf->end_of_buf))
	{			// if it is full
	  if ((--(C->inbuf->refs)) == 0)
	    {			// dereference active-inbuf
    ...
    {				// no inbuf, implies just starting
      C->inbuf = new_inbuf ();	// get new
      assert (C->inbuf);
      C->inbuf->refs = 1;	// add active-inbuf reference
      C->in = C->inbuf->buf;
  ...
  if (C->file)
    {				// if there is an existing active input file
      if (C->insi == NLL && feof (C->file))
	{			//    if it is at EOF
...
// FIXME - stall more more input 
  ...
  if (!C->file)
    {				// if there is no active input file
      if (*(C->pargc))
	{			//   then try to open the next file
	  C->filename = C->argv[0];
	  (*(C->pargc))--;
	  C->argv = &(C->argv[1]);
...
static void
parse_comment_fragment (context_t * C)
{
  unsigned char *in, c;
...
parse_comment (context_t * C)
{
  ...
  while (C->insi == NLL)
    {				// end_of_buffer, or EOF, during comment
...
static void
parse_whitespace_fragment (context_t * C)
{
  unsigned char *in, c;
  state_t insi;
...
parse_non_comment (context_t * C)
{
  ...
  while (C->insi == NLL)
    {				// end_of_buffer, or EOF, during whitespace
      if ((rc = more_in (C) == FAIL))
	{
	  break;		// EOF
	}
...
static int
parse_string_fragment (context_t * C, elem_t * fraglist)
{
  unsigned char *frag;
  state_t insi;
  int slen, len;
...
success_t
parse_string (context_t * C, elem_t * fraglist)
{
  success_t rc;
  int len, slen;
